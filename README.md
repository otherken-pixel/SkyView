...
<body>
    ...
    <script type="text/babel">
        ...
        // Ensure we handle ANY valid 4-letter ICAO
        function getAirport(icao) {
            if (!icao) return null;
            icao = icao.toUpperCase();
            if (AIRPORTS[icao]) return AIRPORTS[icao];
            
            // Generate a plausible mock airport if user typed an unlisted ICAO
            if (icao.length === 4) {
                var mockLat = 25 + (Math.random() * 24); 
                var mockLon = -125 + (Math.random() * 58);
                var newApt = {
                    name: icao + " Facility", city: "Anytown", state: "US",
                    lat: mockLat, lon: mockLon, elev: Math.floor(Math.random() * 5000) + 10,
                    rwy: ["18/36", "09/27"], artcc: "Center",
                    freq: { atis: "120.0", twr: "118.0", gnd: "121.9", app: "124.0", dep: "125.0", clnc: "121.5" }
                };
                AIRPORTS[icao] = newApt; 
                return newApt;
            }
            return null;
        }
        ...
        function makeMetar(icao) {
            ...
            return {
                ...
                wind: wind, vis: vis, sky: sky.raw, temp: tempStr, alt: alt, cat: cat, wspd: wspd
            };
        }
        ...
        function RouteMap({ dep, arr, wps }) {
            var depApt = getAirport(dep);
            var arrApt = getAirport(arr);
            var wpApts = (wps || []).map(function(w) { return getAirport(w); }).filter(Boolean);
            var allPts = [depApt].concat(wpApts).concat([arrApt]).filter(Boolean);

            if (allPts.length < 2) {
                return (
                    <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", background: "var(--md-sys-color-surface-container-lowest)", borderRadius: "16px", minHeight: "300px" }}>
                        <span style={{ color: "var(--md-sys-color-on-surface-variant)", fontSize: "14px" }}>Enter valid departure and destination to view route</span>
                    </div>
                );
            }

            var lats = allPts.map(p => p.lat);
            var lons = allPts.map(p => p.lon);
            var pad = 2.5;
            var minLat = Math.min.apply(null, lats) - pad;
            var maxLat = Math.max.apply(null, lats) + pad;
            var minLon = Math.min.apply(null, lons) - pad;
            var maxLon = Math.max.apply(null, lons) + pad;
            var W = 700, H = 380;

            function project(lat, lon) {
                return {
                    x: ((lon - minLon) / (maxLon - minLon)) * (W - 60) + 30,
                    y: ((maxLat - lat) / (maxLat - minLat)) * (H - 60) + 30
                };
            }

            var codes = [dep].concat(wps || []).concat([arr]);
            var mapPts = allPts.map((p, i) => {
                var proj = project(p.lat, p.lon);
                return { x: proj.x, y: proj.y, code: codes[i], isDep: i === 0, isArr: i === allPts.length - 1 };
            });

            var pathD = mapPts.map((p, i) => (i === 0 ? "M" : "L") + p.x + "," + p.y).join(" ");
            var totalDist = 0;
            for (var i = 0; i < allPts.length - 1; i++) {
                totalDist += calcDist(allPts[i].lat, allPts[i].lon, allPts[i + 1].lat, allPts[i + 1].lon);
            }
            var heading = calcBearing(allPts[0].lat, allPts[0].lon, allPts[allPts.length - 1].lat, allPts[allPts.length - 1].lon);

            return (
                <div style={{ width: "100%", height: "100%", position: "relative", background: "var(--md-sys-color-surface-container-lowest)", borderRadius: "16px", overflow: "hidden", minHeight: "300px" }}>
                    <svg width="100%" height="100%" viewBox={"0 0 " + W + " " + H} preserveAspectRatio="xMidYMid meet">
                        <defs>
                            <pattern id="mapgrid" width="40" height="40" patternUnits="userSpaceOnUse">
                                <path d="M40 0L0 0 0 40" fill="none" stroke="var(--md-sys-color-surface-variant)" strokeWidth="0.5" />
                            </pattern>
                        </defs>
                        <rect width={W} height={H} fill="url(#mapgrid)" />
                        <path d={pathD} fill="none" stroke="var(--md-sys-color-primary)" strokeWidth="3" strokeDasharray="6 4" />
                        
                        {mapPts.map((p, idx) => {
                            var color = p.isDep ? "var(--md-sys-color-primary)" : p.isArr ? "var(--md-sys-color-tertiary)" : "var(--md-sys-color-secondary)";
                            var rad = (p.isDep || p.isArr) ? 6 : 4;
                            return (
                                <g key={idx}>
                                    <circle cx={p.x} cy={p.y} r={rad + 6} fill={color} opacity="0.15" />
                                    <circle cx={p.x} cy={p.y} r={rad} fill={color} />
                                    <text x={p.x} y={p.y - 14} textAnchor="middle" fill="var(--md-sys-color-on-surface)" fontSize="12" fontWeight="700" className="font-mono">{p.code}</text>
                                </g>
                            );
                        })}
                    </svg>
                    <div className="font-mono" style={{ position: "absolute", bottom: 16, left: 16, display: "flex", gap: 8, fontSize: 12 }}>
                        <div style={{ background: "var(--md-sys-color-surface)", borderRadius: "8px", padding: "6px 12px", color: "var(--md-sys-color-on-surface)", boxShadow: "0 1px 3px rgba(0,0,0,0.12)" }}>
                            <span style={{ color: "var(--md-sys-color-on-surface-variant)" }}>DIST </span><span style={{ fontWeight: 700 }}>{Math.round(totalDist)} NM</span>
                        </div>
                        <div style={{ background: "var(--md-sys-color-surface)", borderRadius: "8px", padding: "6px 12px", color: "var(--md-sys-color-on-surface)", boxShadow: "0 1px 3px rgba(0,0,0,0.12)" }}>
                            <span style={{ color: "var(--md-sys-color-on-surface-variant)" }}>HDG </span><span style={{ fontWeight: 700 }}>{String(Math.round(heading)).padStart(3, "0")}&deg;</span>
                        </div>
                    </div>
                </div>
            );
        ...
        function ForecastStrip({ icao, selectedDay, onSelect }) {
            var forecasts = useMemo(() => makeForecast(icao), [icao]);
            return (
        ...
        function WeatherDetail({ icao, label, expanded, setExpanded }) {
            var metar = useMemo(() => makeMetar(icao), [icao]);
            var taf = useMemo(() => makeTaf(icao), [icao]);
            var apt = getAirport(icao);
            if (!metar || !apt) { return null; }
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                        <div>
                            ...
                            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                ...
                                <span style={{ fontSize: 14, color: "var(--md-sys-color-on-surface-variant)" }}>{apt.city}, {apt.state}</span>
                            </div>
                        </div>
                        <CatBadge cat={metar.cat} />
                    </div>
        ...
        function AIBriefing({ trip }) {
            ...
            var runBriefing = useCallback(async function() {
                ...
                var dA = getAirport(trip.dep); var aA = getAirport(trip.arr);
                ...
                var prompt = `TRIP: ${ac.name} (${ac.cat}) cruise ${ac.kts}kt\n` +
                  ...
                  `METAR ${trip.dep}: ${(makeMetar(trip.dep)?.raw || "N/A")}\n` +
                  `TAF ${trip.dep}: ${makeTaf(trip.dep)}\n` +
                  `METAR ${trip.arr}: ${(makeMetar(trip.arr)?.raw || "N/A")}\n`;
                ...
                const payload = {
                    ...
                    systemInstruction: { 
                        parts: [{ 
                            text: "You are an expert aviation weather analyst providing a flight briefing. Use bullet points. Provide the briefing using EXACTLY these headers on their own lines: **GO/NO-GO RECOMMENDATION**, **WEATHER SYNOPSIS**, **DEPARTURE WINDOW**, **EN ROUTE HAZARDS**, **RUNWAY ANALYSIS**, **NOTAM IMPACT**, **ALTERNATE RECOMMENDATIONS**, **FREQUENCY GUIDE**"
                        }] 
                    }
                };
                ...
                try {
                    ...
                    setError("Briefing failed. Please try again later.");
                } finally {
                    setLoading(false);
                }
            }, [trip]);
            ...
            return (
                <div style={{ paddingBottom: '32px' }}>
                    <MdButton 
                        ...
                        onClick={runBriefing} 
                        disabled={loading || !trip.dep || !trip.arr} 
                        icon="auto_awesome"
                        style={{ width: "100%", padding: "16px", marginBottom: "24px", fontSize: "16px" }}
                    >
                        {loading ? "Analyzing Aviation Data..." : "Generate AI Briefing"}
                    </MdButton>
                    ...
                    {!result && !loading && (
                        <div style={{ textAlign: "center", padding: "48px 24px", color: "var(--md-sys-color-on-surface-variant)" }}>
                            ...
                            <div style={{ fontSize: 16, maxWidth: "400px", margin: "0 auto", lineHeight: 1.5 }}>
                                Google AI will analyze METARs, TAFs, NOTAMs, PIREPs, and winds to generate a comprehensive flight briefing.
                            </div>
                        </div>
                    )}
                </div>
        ...
        function App() {
            ...
            var [trips, setTrips] = useState([
                ...
                { id: 2, dep: "KJFK", arr: "KBOS", aircraft: "sr22", date: "2026-03-01", time: "14:30", wps: [], name: "New York to Boston", goScore: null },
            ]);
            ...
            var [depTafOpen, setDepTafOpen] = useState(false);
            var [arrTafOpen, setArrTafOpen] = useState(false);

            useEffect(() => {
                setTrips(prev => prev.map(t => {
                    if (t.goScore) return t;
                    return { ...t, goScore: getAirport(t.dep) && getAirport(t.arr) ? calcGoScore(t.dep, t.arr) : 50 };
                }));
            }, []);

            function createTrip() {
                var validRoute = newTrip.route.filter(r => r && r.length === 4).map(r => r.toUpperCase());
                if (validRoute.length < 2) return;
                
                validRoute.forEach(r => getAirport(r));
            ...
            function openTrip(trip) {
                trip.goScore = calcGoScore(trip.dep, trip.arr);
                setActiveTrip(trip); setTab("brief"); setForecastDay(0);
                setDepTafOpen(false); setArrTafOpen(false); setView("briefing");
            }
                ...
                <div style={{ display: "flex", flexDirection: "column", height: "100vh", overflow: "hidden" }}>
                    ...
                    <main style={{ flex: 1, overflowY: "auto", background: "var(--md-sys-color-background)" }}>
                            ...
                            <div style={{ maxWidth: 840, margin: "0 auto", padding: "32px 16px" }}>
                                ...
                                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                                    {trips.map(trip => {
                                        var dA = getAirport(trip.dep); var aA = getAirport(trip.arr);
                                        var ac = AIRCRAFT.find(a => a.id === trip.aircraft) || AIRCRAFT[0];
                                        var d = dA && aA ? Math.round(calcDist(dA.lat, dA.lon, aA.lat, aA.lon)) : 0;
                                        var ete = dA && aA && ac ? calcDist(dA.lat, dA.lon, aA.lat, aA.lon) / ac.kts : 0;
                                        return (
                                                    ...
                                                    <div style={{ flex: 1, padding: "16px 20px" }}>
                                                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                                                            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                                                                <span className="font-mono" style={{ fontSize: 20, fontWeight: 700, color: "var(--md-sys-color-primary)" }}>{trip.dep}</span>
                                                                <MdIcon name="arrow_right_alt" style={{ color: "var(--md-sys-color-outline)" }} />
                                                                <span className="font-mono" style={{ fontSize: 20, fontWeight: 700, color: "var(--md-sys-color-tertiary)" }}>{trip.arr}</span>
                                                            </div>
                                                        ...
                                                        <div style={{ display: "flex", alignItems: "center", gap: 16, fontSize: 14, color: "var(--md-sys-color-on-surface-variant)", flexWrap: "wrap" }}>
                                                            {dA && <span>{dA.city} &rarr; {aA.city}</span>}
                                                            <span className="font-mono" style={{ background: 'var(--md-sys-color-surface-variant)', padding: '2px 8px', borderRadius: '4px', fontSize: 12 }}>{d} NM</span>
                        ...
                        {view === "create" && (
                            <div style={{ maxWidth: 600, margin: "0 auto", padding: "32px 16px" }}>
                                ...
                                <MdCard style={{ padding: 24 }}>
                                                ...
                                                <div key={i} style={{ display: "flex", alignItems: "flex-end", gap: 12 }}>
                                                    ...
                                                    <MdInput 
                                                        ...
                                                        onChange={e => {
                                                            ...
                                                            newRoute[i] = e.target.value.toUpperCase();
                                    ...
                                    {(() => {
                                        var validRoutePts = newTrip.route.filter(r => r && r.length === 4 && getAirport(r));
                        ...
                        {view === "briefing" && activeTrip && (() => {
                            var dA = getAirport(activeTrip.dep); var aA = getAirport(activeTrip.arr);
                                        ...
                                        <div style={{ padding: "16px 24px", display: "flex", alignItems: "center", gap: 24, borderBottom: "1px solid var(--md-sys-color-surface-variant)" }}>
                                            ...
                                            <div>
                                                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 4 }}>
                                                    <span className="font-mono" style={{ fontSize: 22, fontWeight: 700, color: "var(--md-sys-color-primary)" }}>{activeTrip.dep}</span>
                                                    <MdIcon name="arrow_right_alt" style={{ color: "var(--md-sys-color-outline)" }} />
                                                    <span className="font-mono" style={{ fontSize: 22, fontWeight: 700, color: "var(--md-sys-color-tertiary)" }}>{activeTrip.arr}</span>
                                                </div>
                                        ...
                                        <div style={{ flex: 1, overflowY: "auto", padding: "24px", maxWidth: "900px", margin: "0 auto", width: "100%" }}>
                                            ...
                                            {tab === "wx" && (
                                                <div style={{display: 'flex', flexDirection: 'column', gap: '24px'}}>
                                                    <div>
                                                        <h2 style={{fontSize: 16, fontWeight: 500, margin: "0 0 12px", color: "var(--md-sys-color-on-surface)"}}>Departure</h2>
                                                        <WeatherDetail icao={activeTrip.dep} label="Departure" expanded={depTafOpen} setExpanded={setDepTafOpen} />
                                                        <ForecastStrip icao={activeTrip.dep} selectedDay={forecastDay} onSelect={setForecastDay} />
                                                    </div>
                                                    <div>
                                                    ...
                                                    <div>
                                                        <h2 style={{fontSize: 16, fontWeight: 500, margin: "0 0 12px", color: "var(--md-sys-color-on-surface)"}}>Destination</h2>
                                                        <WeatherDetail icao={activeTrip.arr} label="Destination" expanded={arrTafOpen} setExpanded={setArrTafOpen} />
                                                        <ForecastStrip icao={activeTrip.arr} selectedDay={forecastDay} onSelect={setForecastDay} />
                                                    </div>
                                                </div>
                                            )}
                                            ...
                                            {tab === "notam" && (
                                                <div style={{display: 'flex', flexDirection: 'column', gap: '24px'}}>
                                                    <div>
                                                        <h2 style={{fontSize: 16, fontWeight: 500, margin: "0 0 12px", color: "var(--md-sys-color-on-surface)"}}>NOTAMs - {activeTrip.dep}</h2>
                                                        <NotamsPanel icao={activeTrip.dep} />
                                                    </div>
                                                    <div>
                                                        <h2 style={{fontSize: 16, fontWeight: 500, margin: "0 0 12px", color: "var(--md-sys-color-on-surface)"}}>NOTAMs - {activeTrip.arr}</h2>
                                                        <NotamsPanel icao={activeTrip.arr} />
                                                    </div>
                                                </div>
                                            )}
                                            ...
                                            {tab === "freq" && (
                                                <div style={{display: 'flex', flexDirection: 'column', gap: '24px'}}>
                                                    <FreqPanel icao={activeTrip.dep} />
                                                    <FreqPanel icao={activeTrip.arr} />
                                                </div>
                                            )}
